...
class Periods extends React.Component {
	...
	Years(start, end) {
		...
		this.types.forEach((type, k) => {
			...
			if (results[type])
				resultsText.push(results[type]
					+ ' '
					+ (results[type] !== 1
						? this.props.Locale.com[type.toUpperCase()]
						: this.props.Locale.com[type.slice(0, -1).toUpperCase()]).toLowerCase());
	...
	render() {
		return this.props._
			...
			? <div className={'section-periods'
				+ (this.props.className ? ' ' + this.props.className : '')}>
				{this.props._.map((period, k) => {
					return <div key={k}
						className={'section-period-' + k + ' mb-5'
						...
						<div className="row">
							<div className="col-12 col-md-5 col-lg-4 col-xl-3">
								<p className="h6 d-inline d-md-block fw-bold my-0 me-2">
									{this.dateFormatter[period.dates[0].length > 4 ? 'yearsMonths' : 'years'].format(new Date(period.dates[0]))
										+ (period.dates[0].length > 4
											&& period.dates[1]
											? ' - '
											+ (period.dates[1] !== 'NOW'
												? this.dateFormatter.yearsMonths.format(new Date(period.dates[1]))
												: this.props.Locale.com[period.dates[1]])
											: '')}
								</p>
								{period.dates[1]
									? <p className="small d-inline d-md-block my-0">
										{this.Years(period.dates[0], period.dates[1])}
									</p>
									: null}
							</div>
