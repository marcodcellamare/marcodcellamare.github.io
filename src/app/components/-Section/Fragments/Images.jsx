...
class Images extends React.Component {
	...
	BlobShape() {
		if (this.ref && this.props.blob) {
			const shape = blobshape({
				size: this.state.mounted ? this.state.blob.width : 1,
				growth: Math.round(
					Math.random() *
						(this.state.blob.growth.max -
							this.state.blob.growth.min) +
						this.state.blob.growth.min
				),
				edges: 5,
				seed: null,
	...
	render() {
		return this.props.files && this.props.files.length > 0 ? (
			<div
				ref={(e) => (this.ref = e)}
				className={
					'section-images position-relative w-100' +
				...
				{this.state.mounted ? (
					<div className='section-images-wrapper'>
						{this.props.blob && this.state.blob.shape ? (
							<svg
								...
								<path d={this.state.blob.shape.path} />
								<clipPath id={this.id}>
									<path d={this.state.blob.shape.path} />
								</clipPath>
							</svg>
						...
						<div
							className='section-images-slide position-relative'
							style={
								this.props.blob && this.state.blob.shape
