...
class Counter extends React.Component {
	constructor(props) {
		...
		this.types = ['years', 'months', 'days', 'hours', 'minutes', 'seconds'];
		this.seconds = {
			years: 60 * 60 * 24 * 365,
			months: 60 * 60 * 24 * 30,
			days: 60 * 60 * 24,
	...
	Update() {
		...
		this.types.forEach((type, k) => {
			...
			if (results[type] > 0) {
				if (br) {
					if (!brFound && br === type) brFound = true;
					else if (brFound) postBr.push(type);
				}
				show.push(type);
			} else {
				if (br && !brFound && br === type)
		...
		return this.state.show.length > 0 ? (
			<p
				...
				{this.props.prefx ? (
					<span
						className={
							'counter-item counter-prefx' +
							(this.props.classNamePreBr
								? ' ' + this.props.classNamePreBr
								: '')
						}>
						{this.props.prefx + ' '}
				...
				{this.state.show.map((type, k) => {
					return (
						<React.Fragment key={k}>
							<span
								className={
									'counter-item counter-count counter-count-' +
									k +
									' counter-count-' +
									type +
				...
				{this.props.suffx ? (
					<span
						...
						{' ' +
							this.props.suffx.charAt(0).toLowerCase() +
							this.props.suffx.slice(1)}
						.
					</span>
				) : null}
			</p>
